...
rawdata <- 
  mutate(rawdata,Cluster_k3var3=kmeans_out$cluster |> as.factor())
rawdata |> 
  ggplot(aes(species,fill=Cluster_k3var3))+
  geom_bar()
rawdata |> 
  ggplot(aes(fill=species,x=Cluster_k3var3))+
  geom_bar(position = 'fill')+
  scale_y_continuous(name = 'Frequency', labels=scales::percent)

rawdata |> 
  ggplot(aes(flipper_length_mm,bill_length_mm,
             shape=species,color=Cluster_k3var3))+
  geom_point()
rawdata |> 
  ggplot(aes(flipper_length_mm,bill_length_mm,
             color=species,shape=Cluster_k3var3))+
  geom_point()
